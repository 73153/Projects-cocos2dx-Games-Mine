...
MainScene::init()
{
    ...
    __pScoreLabel = CCLabelTTF::labelWithString("Score: 0", "Arial", 50);
	...
	__pBg1 = CCSprite::spriteWithSpriteFrameName("bg.png");
 ...
 */
void
MainScene::updateEnemies(CCRect balloonRect, CCMutableArray<Enemy*>* pEnemies)
{
	...
	CCMutableArray<Enemy*>* pEnemiesToRemove = new CCMutableArray<Enemy*>();
    ...
    if(pEnemies && pEnemies->count() > 0)
    {
		for(it = pEnemies->begin(); it != pEnemies->end(); it++)
		{
			...
			if(pEnemy)
			{
				...
				bool collision = CCRect::CCRectIntersectsRect(balloonRect, rect);
				if(((CCRect::CCRectGetMinX(rect) < 0) && (CCRect::CCRectGetMaxX(rect) < 0))
						|| (collision))
				{
		...
		// Clean the array.
		pEnemiesToRemove->removeAllObjects(false);
    }
