...
Balloon::init(cocos2d::CCLayer& layer)
{
	...
	String frameNamePrefix = "balloon";
	String textureFileName = "balloons.png";
	int frameNumber = 3;
	float delay = 0.5f;
	__speed = 1000.0f;
	...
	schedule(schedule_selector(Balloon::move), (ccTime)0.03f);
}
...
Balloon::move(ccTime dt)
{
	...
	if(__pSprite)
	{
		...
		if(Math::Abs(xMove * 10) < 5)
		{
			xMove = 0;
		}

		if(Math::Abs(yMove * 10) < 5)
		{
			yMove = 0;
		}
		...
		__pMoveAction = CCMoveTo::actionWithDuration((ccTime)moveDuration, newPosition);
