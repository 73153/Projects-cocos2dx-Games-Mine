...
//  Snake_XT
//
//  Created by Pavitra on 27/09/12.
...
- (void)ccTouchMoved:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if (state_ == kCCScrollLayerStateSliding)
	{
		...
		if ((page == 0 && offset > 0) || (page == [layers_ count] - 1 && offset < 0))
			...
			offset = 0;
        self.position =  ccp(desiredX , offset);

       /* if(!CGPointEqualToPoint(self.position,CGPointMake(-300, self.position.y))){
            self.position = ccpAdd(ccp(self.position.x,self.position.y), ccp(desiredX+20 , offset));
        }
        else{
            self.position = ccpAdd(ccp(self.position.x,self.position.y), ccp(desiredX , offset));
        }
        if(self.position.x>40){
            
            CGPoint pos=ccp(40, self.position.y);
            self.position=pos;
        }
