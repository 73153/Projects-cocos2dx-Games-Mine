...
-(id)initWithRect:(CGRect)rect
{
	...
	if(self){
		...
		nonZeroVelocity = CGPointMake(0,0.2);
		autoCenter = YES;
		isDPad = NO;
		hasDeadzone = NO;
		isActive = NO;
...
-(void)updateVelocity:(CGPoint)point
{
	//nonZeroVelocity Holds The Old Velocity
	nonZeroVelocity = velocity;
...
- (void)ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if(!autoCenter)
    {
        CGPoint location = [[CCDirector sharedDirector] convertToGL:[touch locationInView:[touch view]]];
		location = [self convertToNodeSpace:location];
		//NSLog(@"location at ended is %@",NSStringFromCGPoint(location));
	}
	[self updateVelocity:location];
	isActive = NO;
}
